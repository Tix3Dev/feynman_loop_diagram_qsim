...
impl TriDecomposer {
    ...
    /// Decompose a graph sequentially, returning the statevec and total number of terms.
    pub fn decomp_sequential_statevec(&self, g: impl GraphLike) -> (Vec<Complex<f64>>, usize) {
        let mut g = g;
        self.simplify(&mut g);

        let mut stack = vec![g];
        let e_cnt = 6;
        let mut statevec: Vec<Complex<f64>> =
            vec![num::complex::Complex::new(0.0, 0.0); 2usize.pow(e_cnt)];
        let mut nterms = 0;
        ...
        while !stack.is_empty() {
            ...
            if best_decomp.alpha(&g) < f32::INFINITY {
                ...
                // vector sum a + b
                let a = statevec;
                let b = g.to_tensorf().into_raw_vec();
                statevec = a.iter().zip(b.iter()).map(|(&x, &y)| x + y).collect();
                nterms += 1;
            }
        }
