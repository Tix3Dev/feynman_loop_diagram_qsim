...
pub fn check_remove_id(g: &impl GraphLike, v: V) -> bool {
    ...
    if g.degree(v) == 2 && g.phase(v).is_zero() && (vt == VType::Z || vt == VType::X) {
        let neighbours = g.neighbor_vec(v);
        match (g.edge_type(v, neighbours[0]), g.edge_type(v, neighbours[1])) {
            (EType::T, EType::H) | (EType::H, EType::T) => return false,
            (EType::T, EType::T) => return false,
            _ => return true,
        }
    }
...
pub fn remove_id_unchecked(g: &mut impl GraphLike, v: V) {
    ...
    let new_et = match (nhd[0].1, nhd[1].1) {
        ...
        (EType::N, EType::T) => EType::T,
        (EType::H, EType::N) => EType::H,
        (EType::H, EType::H) => EType::N,
        (EType::T, EType::N) => EType::T,
        (EType::T, EType::H) | (EType::H, EType::T) => {
            panic!("Cannot remove identity between star and hadamard edge")
        }
        (EType::T, EType::T) => panic!("Cannot remove identity between star edges"),
    };
...
pub fn check_color_change(g: &impl GraphLike, v: V) -> bool {
    ...
    (vt == VType::X || vt == VType::Z)
        && g.incident_edges(v)
            .all(|(_, et)| et == EType::N || et == EType::H)
}
...
/// Check [tri_edge_unchecked] applies
pub fn check_tri_edge(g: &impl GraphLike, v: V) -> bool {
    let edges = g.incident_edge_vec(v);

    if edges.len() != 1 || edges[0].1 != EType::T {
        return false;
    }

    let phase = g.phase(v);
    match g.vertex_type(v) {
        VType::Z => return phase.is_one(),
        VType::X => return phase.is_integer(),
        _ => return false,
    }
}

/// Perform basis change for spiders with a single triangle leg.
pub fn tri_edge_unchecked(g: &mut impl GraphLike, v: V) {
    let w = g.neighbor_vec(v)[0];
    g.set_edge_type(v, w, EType::N);

    let phase = g.phase(v);
    match g.vertex_type(v) {
        VType::Z => {
            if phase.is_one() {
                g.set_vertex_type(v, VType::X);
                g.scalar_mut().mul_sqrt2_pow(-1);
            } else {
                panic!("tri_edge: Unexpected phase");
            }
        }
        VType::X => {
            if phase.is_zero() {
                g.set_vertex_type(v, VType::Z);
                g.scalar_mut().mul_sqrt2_pow(1);
            } else if phase.is_one() {
                g.set_phase(v, Rational::zero());
            } else {
                panic!("tri_edge: Unexpected phase");
            }
        }
        _ => panic!("tri_edge: Unexpected vertex type"),
    }
}

checked_rule1!(check_tri_edge, tri_edge_unchecked, tri_edge);

/// Check [split_neighbor] applies
pub fn check_split_neighbor(g: &impl GraphLike, v: V) -> bool {
    if !g.contains_vertex(v) {
        return false;
    }
    ...
    if edges.len() != 1 || !g.phase(v).is_integer() {
        return false;
    }

    let (w, ety) = edges[0];

    // w should have at least one other neighbors besides v
    if g.degree(w) <= 1 {
        return false;
    }

    match (ety, g.vertex_type(v), g.vertex_type(w)) {
        (EType::N, VType::Z, VType::X) => true,
        (EType::N, VType::X, VType::Z) => true,
        (EType::H, VType::Z, VType::Z) => true,
        (EType::H, VType::X, VType::X) => true,
        _ => false,
    }
}

/// Copy a vertex through its single neighbor, splitting it in the process
pub fn split_neighbor_unchecked(g: &mut impl GraphLike, v: V) {
    let w = g.neighbor_vec(v)[0];

    let new_vty = match g.edge_type(v, w) {
        EType::N => g.vertex_type(v),
        EType::H => {
        ...
        EType::T => panic!("split_neighbor: Unexpected edge type"),
    };

    let v_phase = g.phase(v);
    g.remove_vertex(v);

    for (u, ety) in g.incident_edge_vec(w) {
        let new = g.add_vertex_with_phase(new_vty, v_phase);
        g.add_edge_with_type(new, u, ety);
    }

    let phase = g.phase(w);
    let n = g.degree(w) as i32;
    if v_phase.is_one() {
        g.scalar_mut().mul_phase(phase);
    }
    g.scalar_mut().mul_sqrt2_pow(-n + 1);

    g.remove_vertex(w);
}

checked_rule1!(
    check_split_neighbor,
    split_neighbor_unchecked,
    split_neighbor
);
...
fn unfuse_boundary(g: &mut impl GraphLike, v: V, b: V) {
    ...
    let v1 = g.add_vertex_with_data(vd);
    g.add_edge_with_type(v, v1, g.edge_type(v, b));
    g.add_edge_with_type(v1, b, EType::N);
    g.remove_edge(v, b);

    // println!("current g after:\n{}", g.to_dot());
}
...
pub fn remove_pair_unchecked(g: &mut impl GraphLike, v0: V, v1: V) {
    ...
    // Triangle edge
    if et == EType::T {
        if t0 == VType::Z && t1 == VType::Z {
            // 1 + exp(i*p0) + exp(i*p1)
            *g.scalar_mut() *= ScalarN::one() + ScalarN::from_phase(p0) + ScalarN::from_phase(p1);
        } else if t0 == VType::X && t1 == VType::X {
            // (3 + exp(i*p0) + exp(i*p1) - exp(i*(p1+p2))) / 2
            let mut s = ScalarN::one()
                + ScalarN::one()
                + ScalarN::one()
                + ScalarN::from_phase(p0)
                + ScalarN::from_phase(p1)
                + ScalarN::from_phase(p0 + p1 + Rational::one()); // + pi to negate
            s.mul_sqrt2_pow(-2);
            *g.scalar_mut() *= s;
        } else if t0 == VType::Z && t1 == VType::X {
            // (2 + exp(i*p0) + exp(i*(p0+p1))) / sqrt(2)
            let mut s = ScalarN::one()
                + ScalarN::one()
                + ScalarN::from_phase(p0)
                + ScalarN::from_phase(p0 + p1);
            s.mul_sqrt2_pow(-1);
            *g.scalar_mut() *= s;
        } else if t0 == VType::X && t1 == VType::Z {
            // (2 + exp(i*p1) + exp(i*(p0+p1))) / sqrt(2)
            let mut s = ScalarN::one()
                + ScalarN::one()
                + ScalarN::from_phase(p1)
                + ScalarN::from_phase(p0 + p1);
    ...
    // same color
    else if (t0 == t1 && et == EType::N) || (t0 != t1 && et == EType::H) {
